...
class List(models.Model):
    ...
    def get_admin_moderation_url(self):
        return '%s/admindb.cgi/%s/?adminpw=%s' % (self.main_url, self.name,
                                              self.password)

    def subscribe(self, email, first_name=u'', last_name=u'', send_welcome_msg=False):
        from email.Utils import formataddr

        url = '%s/admin.cgi/%s/members/add' % (self.main_url, self.name)
    ...
    def unsubscribe(self, email):
        url = '%s/admin.cgi/%s/members/remove' % (self.main_url, self.name)
    ...
    def get_all_members(self):
        url = '%s/admin.cgi/%s/members/list' % (self.main_url, self.name)
    ...
    def user_subscribe(self, email, password, language='fr', first_name=u'', last_name=u''):
        ...
        url = '%s/subscribe.cgi/%s' % (self.main_url, self.name)
    ...
    def user_unsubscribe(self, email, language='fr'):
        ...
        url = '%s/options.cgi/%s/%s' % (self.main_url, self.name, email)
